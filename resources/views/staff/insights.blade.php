...
<body class="bg-slate-950 text-slate-200 antialiased min-h-screen">
    ...
    <nav class="relative z-50 w-full bg-[#1e1b4b] backdrop-blur-md border-b border-white/10">
        <div class="container mx-auto px-6 py-4 flex items-center justify-between">
            ...
            <form action="{{ route('logout') }}" method="POST" class="m-0">
                @csrf
                <button type="submit" class="px-5 py-2 rounded-full text-sm bg-slate-800 hover:bg-slate-700 transition-all duration-300 shadow-lg hover:shadow-indigo-500/10 text-white">Logout</button>
            </form>
        </div>
    </nav>
    ...
    <main class="container mx-auto px-6 py-12">
        <header class="mb-12 reveal-on-scroll opacity-0 translate-y-10 transition-all duration-1000 ease-out">
            ...
            <p class="text-slate-400 text-sm">Predictive analytics and operational performance overview.</p>
        </header>

        <!-- Charts Grid -->
        <div class="grid md:grid-cols-2 gap-6">
            <!-- Revenue Trend -->
            <div class="bg-slate-900/40 border border-slate-800/80 rounded-2xl p-8 hover:border-slate-700 transition-all duration-300 hover:shadow-[0_0_20px_rgba(139,92,246,0.1)] reveal-on-scroll opacity-0 translate-y-10 transition-all duration-1000 ease-out">
                <h3 class="font-bold text-white mb-6">Revenue Trend</h3>
                <div class="h-64 flex items-end justify-between gap-2 px-4">
                    @foreach([40, 60, 45, 80, 70, 95, 85] as $height)
                        <div class="w-full bg-violet-500/30 rounded-t-lg transition-all hover:bg-violet-500" style="height: {{$height}}%"></div>
                    @endforeach
                </div>
            </div>
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        <div class="flex justify-between text-sm mb-2">
                            <span class="text-slate-300">{{$service[0]}}</span>
                            <span class="font-medium text-violet-400">{{$service[1]}}%</span>
                        </div>
                        <div class="h-2 bg-slate-800 rounded-full overflow-hidden">
                            <div class="h-full bg-violet-500 transition-all duration-1000" style="width: {{$service[1]}}%"></div>
                        </div>
                    </div>
                    @endforeach
                </div>
