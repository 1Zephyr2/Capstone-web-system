...
<body class="bg-gray-50 text-gray-800 antialiased min-h-screen">
    ...
    <nav class="relative z-50 w-full bg-white border-b border-gray-200 shadow-sm">
        <div class="container mx-auto px-6 py-4 flex items-center justify-between">
            ...
            <div class="hidden md:flex items-center gap-6 text-sm font-medium text-gray-500">
                ...
                <a href="{{ route('admin.directory') }}"    class="text-gray-900 font-semibold transition-all hover:scale-105">Pets</a>
    ...
    <main class="container mx-auto px-6 py-12">
        ...
        <div class="mb-5 max-w-lg reveal-on-scroll opacity-0 translate-y-10 transition-all duration-700 ease-out">
            <div class="relative">
                <i class="bi bi-search absolute left-4 top-3 text-gray-500"></i>
                <input id="search" type="text" placeholder="Search by owner name or pet name..."
                       class="w-full bg-white border border-gray-200 rounded-xl pl-12 pr-4 py-3 text-gray-900 placeholder-indigo-500 focus:border-indigo-500 transition-all outline-none text-sm">
            </div>
        </div>
        ...
        <!-- Stats -->
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8 reveal-on-scroll opacity-0 translate-y-10 transition-all duration-700 ease-out">
        ...
        <div class="space-y-3">
            @forelse($owners as $owner)
                ...
                <div class="owner-card bg-white border {{ $owner->is_inactive ? 'border-red-200' : 'border-gray-200' }} rounded-xl overflow-hidden hover:border-indigo-300 transition-all duration-300 reveal-on-scroll opacity-0 translate-y-10 transition-all duration-700 ease-out"
                    ...
                    <button onclick="toggleAccordion({{ $owner->id }})"
                            class="w-full px-6 py-4 flex items-center justify-between text-left hover:bg-gray-50 transition-all">
                        <div class="flex items-center gap-3">
                            <div class="w-9 h-9 rounded-full {{ $owner->is_inactive ? 'bg-red-100' : 'bg-indigo-100' }} flex items-center justify-center {{ $owner->is_inactive ? 'text-red-600' : 'text-gray-500' }} shrink-0">
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <i id="icon-{{ $owner->id }}" class="bi bi-chevron-right text-gray-500 transition-transform duration-300"></i>
                        </div>
                    </button>
                        ...
                        <div class="pt-4 space-y-2">
                            @forelse($owner->pets as $pet)
                                <a href="{{ route('pets.details', ['id' => $pet->id]) }}"
                                   class="flex items-center justify-between bg-gray-50/50 p-3 rounded-lg border border-gray-200 hover:border-indigo-200 transition-all hover:translate-x-1">
                                    <div class="flex items-center gap-3">
                                        @if($pet->photo)
