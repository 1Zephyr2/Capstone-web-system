@php
    ...
    if ($isAdmin) {
        ...
        $inputBg='bg-indigo-950 border-indigo-700'; $portalLabel='ADMIN PORTAL';
    } else {
        ...
        $accentBtn='bg-violet-600 hover:bg-violet-500';
        $inputBg='bg-slate-950 border-slate-700'; $portalLabel='STAFF PORTAL';
    }
    ...
    <script>
        ...
        function switchTab(tab) {
            ['appointments','records'].forEach(t => {
                ...
                document.getElementById('btn-' + t).classList.remove('border-violet-500','text-white','border-indigo-500');
                document.getElementById('btn-' + t).classList.add('border-transparent','text-slate-400');
            });
            document.getElementById('tab-' + tab).classList.remove('hidden');
            document.getElementById('btn-' + tab).classList.add('{{ $isAdmin ? "border-indigo-500" : "border-violet-500" }}','text-white');
...
<body class="{{ $bgMain }} text-slate-200 antialiased min-h-screen">
    ...
    <nav class="relative z-50 w-full {{ $bgNav }} backdrop-blur-md border-b border-white/10">
        <div class="container mx-auto px-6 py-4 flex items-center justify-between">
            ...
            <form action="{{ route($isAdmin ? 'admin.logout' : 'staff.logout') }}" method="POST" class="m-0">
                @csrf
                <button class="px-5 py-2 rounded-full text-sm bg-slate-800 hover:bg-slate-700 transition-all text-white">Logout</button>
            </form>
        </div>
    </nav>
    ...
    <main class="container mx-auto px-6 py-10">
        ...
        @if(session('success'))
            <div class="mb-5 px-5 py-3 rounded-xl bg-emerald-500/10 border border-emerald-500/30 text-emerald-300 flex items-center gap-3 text-sm">
        ...
        <header class="mb-8 flex items-start justify-between reveal-on-scroll opacity-0 translate-y-10 transition-all duration-700 ease-out">
            <div class="flex items-center gap-5">
                @if($pet->photo_url)
                    <img src="{{ $pet->photo_url }}" alt="{{ $pet->name }}" class="w-20 h-20 rounded-2xl object-cover border-2 {{ $isAdmin ? 'border-indigo-500/30' : 'border-violet-500/30' }} shadow-lg">
                @else
                    <div class="w-20 h-20 rounded-2xl {{ $isAdmin ? 'bg-indigo-500/20 border-indigo-500/30' : 'bg-violet-500/20 border-violet-500/30' }} flex items-center justify-center text-3xl border">
            ...
            <a href="{{ route($prefix.'.directory') }}" class="px-4 py-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-white text-sm font-semibold transition-all">← Back</a>
        </header>
        ...
        <div class="grid lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-5">
                ...
                <!-- Tab Buttons -->
                <div class="flex gap-1 {{ $cardBg }} border {{ $cardBorder }} rounded-xl p-1">
                ...
                <div id="tab-appointments">
                    ...
                    <div class="{{ $cardBg }} border {{ $cardBorder }} rounded-2xl p-5 mb-4">
                    ...
                    <div class="{{ $cardBg }} border {{ $cardBorder }} rounded-2xl overflow-hidden">
                        ...
                        <div class="overflow-x-auto">
                            <table class="w-full text-sm">
                                <thead class="bg-slate-950/40 border-b {{ $cardBorder }}">
                                ...
                                <tbody class="divide-y divide-slate-800/40">
                                    @foreach($completedAppointments as $appt)
                                        <tr class="hover:bg-slate-800/20 transition-all cursor-pointer"
                                            ...
                                            <td class="px-5 py-3 text-slate-300 whitespace-nowrap">{{ $appt->appointment_date->format('M d, Y') }}</td>
                                            <td class="px-5 py-3 text-white font-medium">{{ $appt->service_label }}</td>
                                            <td class="px-5 py-3 text-slate-400 text-xs max-w-[150px] truncate">{{ $appt->notes ?: '—' }}</td>
                            ...
                            <div class="grid grid-cols-2 gap-3">
                                <div>
                                    ...
                                    <input type="date" name="record_date" value="{{ date('Y-m-d') }}" required
                                           class="w-full {{ $inputBg }} border rounded-xl px-4 py-2.5 text-white outline-none focus:border-violet-500 transition-all text-sm">
                                </div>
                                <div>
                                    <label class="block text-xs font-semibold uppercase tracking-wider text-slate-500 mb-1">Diagnosis</label>
                                    <input type="text" name="diagnosis" required placeholder="e.g. Skin infection"
                                           class="w-full {{ $inputBg }} border rounded-xl px-4 py-2.5 text-white outline-none focus:border-violet-500 transition-all text-sm">
                                </div>
                            </div>
                            <div>
                                <label class="block text-xs font-semibold uppercase tracking-wider text-slate-500 mb-1">Medications</label>
                                <input type="text" name="medications" placeholder="e.g. Amoxicillin 250mg"
                                       class="w-full {{ $inputBg }} border rounded-xl px-4 py-2.5 text-white outline-none focus:border-violet-500 transition-all text-sm">
                            </div>
                            <div>
                                <label class="block text-xs font-semibold uppercase tracking-wider text-slate-500 mb-1">Vet Notes</label>
                                <textarea name="vet_notes" rows="2" placeholder="Follow-up instructions, observations..."
                                          class="w-full {{ $inputBg }} border rounded-xl px-4 py-2.5 text-white outline-none focus:border-violet-500 transition-all resize-none text-sm"></textarea>
                            </div>
                    ...
                    <div class="{{ $cardBg }} border {{ $cardBorder }} rounded-2xl overflow-hidden">
                                ...
                                <tbody class="divide-y divide-slate-800/40">
                                    @foreach($pet->records as $record)
                                        <tr class="hover:bg-slate-800/20 transition-all">
                                            <td class="px-5 py-3 text-slate-300 whitespace-nowrap">{{ $record->record_date->format('M d, Y') }}</td>
                                            <td class="px-5 py-3 text-white font-medium">{{ $record->diagnosis }}</td>
                                            <td class="px-5 py-3 text-slate-400 text-xs max-w-[140px] truncate">{{ $record->medications ?: '—' }}</td>
                                            <td class="px-5 py-3 text-slate-400 text-xs">{{ $record->recorder->name ?? '—' }}</td>
                                            <td class="px-5 py-3">
                                                <div class="flex items-center gap-2">
                                                    ...
                                                    )" class="p-1.5 rounded-lg {{ $isAdmin ? 'text-indigo-400 hover:bg-indigo-500/20' : 'text-violet-400 hover:bg-violet-500/20' }} transition-all">
                                                        <i class="bi bi-pencil text-xs"></i>
                                                    </button>
                                                    <form method="POST" action="{{ route($prefix . '.records.destroy', $record) }}"
                                                          onsubmit="return confirm('Delete this record?')">
                                                        @csrf @method('DELETE')
                                                        <button type="submit" class="p-1.5 rounded-lg text-red-400 hover:bg-red-500/20 transition-all">
                        ...
                        @else
                            <div class="text-center py-10">
                                ...
                                <p class="text-slate-500 text-sm italic">No medical records yet. Add one above.</p>
                            </div>
                        @endif
                    </div>
                </div>
            ...
            <div class="space-y-5 reveal-on-scroll opacity-0 translate-y-10 transition-all duration-700 ease-out">
                ...
                <div class="{{ $cardBg }} border {{ $cardBorder }} rounded-2xl p-5">
                    ...
                    <p class="text-slate-300 text-sm leading-relaxed">{{ $pet->special_notes ?: 'No special notes.' }}</p>
                </div>
                ...
                <div class="{{ $cardBg }} border {{ $cardBorder }} rounded-2xl p-5">
                    ...
                    <p class="text-white font-semibold text-sm">{{ $pet->user->name }}</p>
                    <p class="text-slate-400 text-xs mt-0.5">{{ $pet->user->email }}</p>
                    <button onclick="showOwnerProfile('{{ addslashes($pet->user->name) }}','{{ addslashes($pet->user->email) }}')"
                            class="w-full mt-3 px-4 py-2 border {{ $cardBorder }} rounded-lg text-xs hover:bg-slate-800 transition-all text-slate-300">
                        View Profile
                    </button>
                </div>
            </div>
    ...
    <div id="visit-modal" class="fixed inset-0 z-50 hidden items-center justify-center p-6 bg-slate-950/80 backdrop-blur-sm transition-opacity duration-300 ease-out"
        ...
        <div id="visit-modal-content" class="{{ $isAdmin ? 'bg-indigo-950 border-indigo-800' : 'bg-slate-900 border-slate-800' }} border rounded-2xl p-8 w-full max-w-md shadow-2xl transform scale-95 opacity-0 transition-all duration-300 ease-out">
            <h2 class="text-xl font-bold text-white mb-5">Appointment Details</h2>
            <div class="space-y-3">
                <div><label class="block text-xs font-semibold uppercase tracking-wider {{ $accentText }} mb-1">Service</label>
                    <p id="visit-service" class="text-white p-3 rounded-lg border {{ $isAdmin ? 'bg-indigo-900 border-indigo-700' : 'bg-slate-950 border-slate-800' }}"></p></div>
                <div><label class="block text-xs font-semibold uppercase tracking-wider {{ $accentText }} mb-1">Date & Time</label>
                    <p id="visit-date" class="text-white p-3 rounded-lg border {{ $isAdmin ? 'bg-indigo-900 border-indigo-700' : 'bg-slate-950 border-slate-800' }}"></p></div>
                <div><label class="block text-xs font-semibold uppercase tracking-wider {{ $accentText }} mb-1">Status</label>
                    <p id="visit-status" class="text-white p-3 rounded-lg border {{ $isAdmin ? 'bg-indigo-900 border-indigo-700' : 'bg-slate-950 border-slate-800' }}"></p></div>
                <div><label class="block text-xs font-semibold uppercase tracking-wider {{ $accentText }} mb-1">Notes</label>
                    <p id="visit-notes" class="text-slate-300 p-3 rounded-lg border text-sm {{ $isAdmin ? 'bg-indigo-900 border-indigo-700' : 'bg-slate-950 border-slate-800' }}"></p></div>
                <button onclick="closeModal('visit-modal','visit-modal-content')" class="w-full mt-2 px-4 py-2.5 rounded-xl bg-slate-800 text-slate-300 hover:bg-slate-700 transition-all">Close</button>
            </div>
        </div>
    </div>

    <!-- Edit Record Modal -->
    <div id="edit-record-modal" class="fixed inset-0 z-50 hidden items-center justify-center p-6 bg-slate-950/80 backdrop-blur-sm transition-opacity duration-300 ease-out"
         onclick="if(event.target===this) closeModal('edit-record-modal','edit-record-modal-content')">
        <div id="edit-record-modal-content" class="{{ $isAdmin ? 'bg-indigo-950 border-indigo-800' : 'bg-slate-900 border-slate-800' }} border rounded-2xl p-8 w-full max-w-md shadow-2xl transform scale-95 opacity-0 transition-all duration-300 ease-out">
            <h2 class="text-xl font-bold text-white mb-5">Edit Medical Record</h2>
            <form id="edit-record-form" method="POST" class="space-y-4">
                ...
                <div class="flex gap-3 pt-1">
                    <button type="button" onclick="closeModal('edit-record-modal','edit-record-modal-content')"
                            class="flex-1 px-4 py-2.5 rounded-xl bg-slate-800 text-slate-300 hover:bg-slate-700 transition-all font-semibold">Cancel</button>
                    <button type="submit"
    ...
    <div id="profile-modal" class="fixed inset-0 z-50 hidden items-center justify-center p-6 bg-slate-950/80 backdrop-blur-sm transition-opacity duration-300 ease-out"
        ...
        <div id="profile-modal-content" class="{{ $isAdmin ? 'bg-indigo-950 border-indigo-800' : 'bg-slate-900 border-slate-800' }} border rounded-2xl p-8 w-full max-w-md shadow-2xl transform scale-95 opacity-0 transition-all duration-300 ease-out">
            <h2 class="text-xl font-bold text-white mb-5">Owner Profile</h2>
            <div class="space-y-3">
                <div><label class="block text-xs font-semibold uppercase tracking-wider {{ $accentText }} mb-1">Name</label>
                    <p id="owner-name" class="text-white p-3 rounded-lg border {{ $isAdmin ? 'bg-indigo-900 border-indigo-700' : 'bg-slate-950 border-slate-800' }}"></p></div>
                <div><label class="block text-xs font-semibold uppercase tracking-wider {{ $accentText }} mb-1">Email</label>
                    <p id="owner-email" class="text-white p-3 rounded-lg border {{ $isAdmin ? 'bg-indigo-900 border-indigo-700' : 'bg-slate-950 border-slate-800' }}"></p></div>
                <button onclick="closeModal('profile-modal','profile-modal-content')" class="w-full mt-2 px-4 py-2 rounded-xl bg-slate-800 text-slate-300 hover:bg-slate-700 transition-all">Close</button>
            </div>
        </div>
    </div>
</body>
